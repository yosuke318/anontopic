...
`{"type":"error","code":"blocked","reason":"contact"}` のように止めた理由の分類が返る。
...
外部連絡先（URL・メールアドレス・電話番号・SNSのID・住所・位置情報）の正規表現で、AI判定は使わない。
...
外部連絡先は辞書とは別の形で読む。「アットマーク」「ドット」などの言い換えを記号に戻し、
漢数字や記号で区切った電話番号を数字の並びに戻してから正規表現に当てる。読み替えの内容と
通ってしまう書き方は [ADR-0019](docs/adr/0019-read-a-contact-detail-from-forms-of-its-own.md) にある。

送信者には止めた理由を `ng_words.category` の分類（外部連絡先は `contact`）で返し、画面は
分類ごとに利用規約で禁止している旨を出す。どの語に当たったかは返さない。理由は
[ADR-0020](docs/adr/0020-tell-the-sender-the-category-a-message-was-blocked-for.md) にある。
