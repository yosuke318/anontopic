...
func (m moderator) Moderate(ctx context.Context, body string) (chat.Decision, error) {
	decision, err := m.filter.Moderate(ctx, body)
	if err != nil || decision == moderation.DecisionBlock {
		return chat.DecisionBlock, err
	}

	return chat.DecisionAllow, nil
}
