...
func TestActiveWordsReadsTheWordsInForce(t *testing.T) {
	...
	if !slices.Contains(words, Word{Text: inForce, Category: CategorySexual}) {
		t.Fatalf("ActiveWords left out %q of %q, want it read with its category", inForce, CategorySexual)
	}
	if slices.ContainsFunc(words, func(w Word) bool { return w.Text == switchedOff }) {
